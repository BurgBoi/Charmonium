    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return parent -> {
            ConfigBuilder builder = ConfigBuilder.create()
                ...
                .setTitle(new TranslatableText("cloth." + getModId() + ".title"));
            ...
            ConfigCategory mainCategory = builder.getOrCreateCategory(new TranslatableText("cloth." + getModId() + ".category.title"));

            for (T module : modules) {
                Map<Field, Object> properties = getModuleConfigProperties(module);

                SubCategoryBuilder subcategory = builder.entryBuilder()
                    .startSubCategory(new LiteralText(module.getName()));

                if (!module.isAlwaysEnabled()) {
                    FieldBuilder<?, ?> enabledValue = builder.entryBuilder()
                        .startBooleanToggle(new TranslatableText("cloth." + getModId() + ".category.module_enabled"), module.isEnabledInConfig())
                        .setDefaultValue(module.isEnabledByDefault()) // Used when user click "Reset"
                        .setTooltip(new TranslatableText(StringHelper.splitOverLines(module.getDescription()))) // Shown when the user hover over this option
                ...
                properties.forEach((prop, value) -> {
                    ...
                    LiteralText name = new LiteralText(annotation.name());
                    LiteralText desc = new LiteralText(StringHelper.splitOverLines(annotation.description()));
